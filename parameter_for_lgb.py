...
lgb_clf = LGBMClassifier(random_state=48)

# 指定待搜索的超参数分布
param_dist = {
    'n_estimators':        randint(100, 500),    # 树数量
    'num_leaves':          randint(20, 150),     # 叶子节点数
    'max_depth':           randint(3, 12),       # 树深度
    'learning_rate':       uniform(0.01, 0.19),  # 学习率
    'subsample':           uniform(0.6, 0.4),    # 行采样比例
    'colsample_bytree':    uniform(0.6, 0.4),    # 列采样比例
    'min_child_samples':   randint(5, 50),       # 最小叶子样本数
    'reg_alpha':           uniform(0, 1),        # L1 正则
    'reg_lambda':          uniform(0, 5)         # L2 正则
}
...
# 执行搜索
rs_lgb.fit(X_train_np, y_train_np.ravel())

# 输出最优参数与精确率
print("Best parameters for LightGBM:")
print(rs_lgb.best_params_)
print(f"Best precision score: {rs_lgb.best_score_:.4f}")